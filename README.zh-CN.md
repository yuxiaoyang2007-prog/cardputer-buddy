...
| 方式 | 用途 | 是否必需 |
|------|------|----------|
| **USB-C 数据线** | 仅首次刷机用（Mac → Cardputer） | 一次性必需 |
| **蓝牙（BLE）** | 运行时通信（Mac 守护进程 ↔ Cardputer，Nordic UART 协议） | 始终必需 |
| **WiFi** | 可选，仅用于开机显示 IP 地址 | 非必需 |

刷机完成后，Cardputer 跟 Mac 之间只走 BLE 蓝牙通信。不需要互联网、不需要 USB、不需要 WiFi。两端都开蓝牙就行，macOS 自带蓝牙。
...
### 4. （可选）配置 statusline 显示用量

如果想在设备上看到 5 小时和每周的用量进度条，在 `~/.claude/settings.json` 加上 statusline 配置：

```json
{
  "statusLine": {
    "type": "command",
    "command": "bash '/你的路径/cardputer/scripts/statusline.sh'"
  }
}
```

脚本会从 Claude Code 的 statusline JSON 里抓 `rate_limits`，写到 `~/.cardputer-daemon/usage.json`，由守护进程转发给设备。如果你已经在用其他 statusline 工具（比如 claude-hud），把它的命令放到 `~/.cardputer-daemon/downstream-statusline.sh`，cardputer 的 wrapper 会自动接力调用。

注意：`rate_limits` 数据只对 Claude.ai 订阅用户（OAuth 登录）有效，且需要在 session 里发送第一条消息之后才能拿到。
